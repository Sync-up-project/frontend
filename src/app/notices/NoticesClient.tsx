      ...
      <div className="mx-auto w-full max-w-[1200px] px-6 lg:px-10 py-8">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">{tr("중요 공지 및 업데이트를 확인해 주세요.", "重要なお知らせと更新情報")}</p>
            </div>
          </div>
        ...
        <div className="mt-6 rounded-xl border border-gray-200 bg-white p-3">
          <input
            ...
            className="w-full rounded-lg border border-gray-200 bg-gray-50 px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-gray-300"
          />
        </div>
        ...
        <div className="mt-4 rounded-xl border border-gray-200 bg-white">
          {loading ? (
            ...
            <div className="px-4 py-10 text-center text-sm text-gray-600">{tr("공지사항이 없습니다.", "お知らせはありません。")}</div>
          ) : (
            <ul className="divide-y divide-gray-100">
              {filtered.map((n) => (
                <li key={n.id} className="px-4 py-4 hover:bg-gray-50">
